...
def get_rows_csv(reader):
    pass
        

def generate_model(category, queries):
    with open('category/{0}.py'.format(category), 'w') as model_file:
        print >> model_file, get_header(category)
        print >> model_file, "#    ------ BEGIN ATTRIBUTES -----    #"
        count = 0
        for query in queries:
            print >> model_file, "#    *** QUESTION {0} ***    ".format(count)
